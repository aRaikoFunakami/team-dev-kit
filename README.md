...
| 秘密情報ガード | commit 前・発行前に gitleaks で機械検査し、危険ならブロック（二重ガード） | pre-commit hook（`.githooks/pre-commit`）＋ PreToolUse egress hook（[`egress-scan.sh`](plugins/team-dev-kit/scripts/egress-scan.sh)） |
