...
> **強制レイヤの注意**: `settings.json` の deny は **Claude エージェントの push を止めるだけ**。
> 人間や他クライアントからの default ブランチへの直接 push を防ぐ強制は **GitHub のブランチ保護**で行う
> （リポジトリ初期設定で一度だけ。手順は team-dev-kit README「default ブランチを保護する」）。
