...
L3 は git 履歴を通らない gh 経路を塞ぐ唯一の層なので、層を減らす議論の対象にはならない。本文をリモートへアップロードする gh コマンド（issue/pr の `create` / `edit` / `comment`、release の `create` / `edit`、`gist create`）を実行する直前に、アップロードされる本文（`--title` / `--body` / `--notes` の値、`--body-file` / `--notes-file` の中身、gist の対象ファイルの中身）だけを gitleaks で走査し、検出時は exit 2 で実行を拒否する。`create` だけ塞いでも `edit` / `comment` / リリースノート経由で同じ秘密情報がアップロードされうるため、本文を載せる経路を一律に対象へ含める。
...
3. `sh tests/smoke.sh` を実行し、検出すべき検体が引き続き検出され、追加した例示値が除外されることを確認する

ドキュメントに実例が要るときは、実値ではなく上記の allowlist 済みダミー値（`/Users/<user>/`・`example.com`・`192.168.0.1`・`090-0000-0000`）を使う。

## 既知の制約

L3 は本文がコマンド引数に乗った対象 gh コマンドだけを走査する。本文指定のない場合は本文を特定できず素通りする。具体的には、`--editor` / `--template` で対話的にエディタへ書く場合（内容がコマンド引数に乗らない）や、コミットから本文を生成する `gh pr create`（PR 本文に `--body` を渡さない）、`gist create` の stdin 入力（`-`）がこれにあたる。対話エディタ経由は人手の操作で自動化の対象外であり、PR の本文をコミットから起こす経路のコミット内容は L1 が守る。自動化（skill 経由）は `--body-file` か `--body` を使うため、この素通り経路には乗らない。

`gh api` の変更系リクエスト（`-X POST/PATCH/PUT/DELETE` やフィールド付与 `-f`/`-F`/`--field`/`--input`）は本文がフィールドや独自スキーマに散らばり確実に抽出できないため、走査漏れを避けて実行を拒否する（fail-closed）。読み取り（GET）はアップロードが無いため素通りする。

秘密情報の egress を止める層は判定不能なら止めるのが原則のため、判定に必要な情報を取りこぼした場合は一律 fail-closed に倒す。入力 JSON（PreToolUse の引数）の parse 失敗、`tool_name` / `tool_input` の欠落など想定外スキーマ、対象 gh と判定した後のコマンド解析（クォート対応の分割）失敗は、いずれも走査対象を特定できないため素通りさせず実行を拒否する。`Bash` 以外・対象 gh 以外と確実に判定できた場合だけ素通りさせる。
...
導入は bootstrap が一括で行う。対象リポジトリのルートで実行すると、`.githooks/pre-commit` の配置、
`.gitleaks.toml`（base 継承の overlay）の配置、`core.hooksPath` の設定までを済ませる。

```sh
brew install gitleaks   # 検出器（未導入なら別途）
curl -fsSL https://raw.githubusercontent.com/aRaikoFunakami/team-dev-kit/main/bootstrap.sh | sh
```

`core.hooksPath = .githooks` を使うのは、pre-commit フレームワーク（`.pre-commit-config.yaml`）が Python と `pre-commit` パッケージへの依存を増やすのを避けるためだ。`.git/hooks` はバージョン管理外なので、コミット済みの `.githooks/` を指すよう設定する。フックスクリプト自体をリポジトリに置いてレビュー対象にできる利点も取った。bootstrap が未設定時にだけこの 1 行を代行する（既に他のフックパスがあれば上書きせず警告する）。
...
L1 が検体（`tests/fixtures/gitleaks-sample.txt` の private IP）でコミットを止め、L3（`egress-scan.sh`）が
PII を含むアップロード（`tests/fixtures/egress-pii.json`）を拒否し、クリーン本文（`egress-clean.json`）と
無関係コマンド（`egress-skip.json`）を素通りさせることを確認する。これらは bootstrap 導入・glue 注入・
冪等性などと合わせて `tests/smoke.sh` の一連のアサーションで検証される。
